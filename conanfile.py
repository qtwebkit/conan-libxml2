...
class Libxml2Conan(ConanFile):
    ...
    def _configure_autotools(self):
        ...
        full_install_subfolder = tools.unix_path(
            self.package_folder) if tools.os_info.is_windows else self.package_folder
        # fix rpath
        ...
        self._autotools.configure(args=configure_args, build=build,
    ...
    def package(self):
        ...
        self.copy("COPYING", src=self._source_subfolder, dst="licenses", ignore_case=True,
                  keep_path=False)
        if self._is_msvc:
            self._package_msvc()
        else:
